...
# Crystal DarkDevil (077.Lib):
# - Attack1 body + blend overlay at 342 + direction * 6
# - ObjectRangeAttack burst at 272 + direction * 8 (8 frames)
# - Die burst at 336 (6 frames)
$actions = [ordered]@{
  ...
  attackRange1 = @{ start = 80; count = 6; offset = 6; interval = 100 }
  ...
  attack1Blend = @{ start = 342; count = 6; offset = 6; interval = 100 }
  dieBlend = @{ start = 336; count = 6; offset = 0; interval = 100 }
  projectile = @{ start = (272 + ($Direction * 8)); count = 8; offset = 0; interval = 30 }
}
...
Write-Output "Built Dark Devil combat atlas $Index ($($frames.Count) slots, ${slotWidth}x${slotHeight})"
