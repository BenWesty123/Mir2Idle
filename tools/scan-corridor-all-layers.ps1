...
for ($x = $x0; $x -le $x1; $x++) {
  for ($y = $cropY; $y -le $endY; $y++) {
    ...
    if ($midFrame -ge 0) {
      ...
      if ($null -ne $image) {
        $results += [pscustomobject]@{ x=$x; y=$y; layer='middle'; slot=1; frame=$midFrame; w=$image.Bitmap.Width; h=$image.Bitmap.Height; floor=(Test-FloorSized $image.Bitmap) }
      }
    }
    ...
    if ($frontFrame -ge 0 -and $frontSlot -ne -1 -and $frontSlot -ne 200) {
      ...
      if ($null -ne $image) {
        $results += [pscustomobject]@{ x=$x; y=$y; layer='front'; slot=$frontSlot; frame=$frontFrame; w=$image.Bitmap.Width; h=$image.Bitmap.Height; floor=(Test-FloorSized $image.Bitmap) }
      }
    }
  }
}
$results | Sort-Object x,y,layer | Format-Table -AutoSize
Write-Host "Total object entries: $($results.Count)"
