param(
  ...
  [string[]]$MapNames = @("D2061.map", "D2062.map", "D2063.map", "D2064.map", "D2065.map", "D2066.map", "D2067.map"),
  [string]$OutputRoot = "../tile-review/prajna-cave-prop-catalog",
...
function Get-VisibleBackFrame([int]$backFrame) {
  # PrajnaStoneCave maps store 1950-1954 on the back layer; visible floor art is 3051-3055.
  if ($backFrame -ge 1950 -and $backFrame -le 1954) { return 3051 + ($backFrame - 1950) }
...
$metadata = [ordered]@{
  title = "Prajna Cave Prop Catalog"
...
<html>
  <head>
    <meta charset="utf-8" />
    <title>Prajna Cave Prop Catalog</title>
    <style>
  ...
  <body>
    <header>
      <h1>Prajna Cave Prop Catalog</h1>
...
Write-Output "Prajna Cave catalog: $($renderedGroups.Count) decorations"
