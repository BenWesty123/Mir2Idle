param(
  ...
  [string]$OutputRoot = "../public/spellfx/FlameDisruptor"
)
...
try {
  # PlayerObject cast: Effect(Libraries.Magic2, 130, 6, ...)
  $cast = Export-SpellLayer -Lib $lib -Start 130 -Count 6 -SheetName "l0.png" -Interval 100
  # PlayerObject impact: Effect(Libraries.Magic2, 140, 10, 600, TargetPoint)
  $impact = Export-SpellLayer -Lib $lib -Start 140 -Count 10 -SheetName "target.png" -Interval 60
  ...
  Write-Output "Exported FlameDisruptor cast=$($cast.frames.Count) impact=$($impact.frames.Count) to $OutputRoot"
}
finally {
