param(
  ...
  [string]$MapPath = "C:/Users/bb-we/Documents/Crystal-master/Next/NextClient/Map/D2002.map",
  [string]$OutputRoot = "../tile-review/bdd-2f-overview",
  [string]$MapTitle = "BlackSnakePalace",
  [string]$MapLabel = "King Scorpion room (Black Snake Palace)",
  [string]$ImagePrefix = "d2002-overview",
  [int]$CropX = 0,
  [int]$CropY = 0,
  [int]$CropWCells = 0,
  [int]$CropHCells = 0,
...
# Crystal BlackSnakePalace (D2002) - King Scorpion room per server GM teleporter.
$markers = @(
  @{ x = 13; y = 25; className = "boss"; title = "King Scorpion (GM teleporter)" },
  @{ x = 30; y = 35; className = "hub"; title = "BDD2 hub warp" },
  @{ x = 17; y = 22; className = "spawn"; title = "Mob respawn cluster" },
  @{ x = 33; y = 39; className = "entry"; title = "Exit to D2001 cave" },
  @{ x = 30; y = 11; className = "entry"; title = "Exit to BDD 1F (D2003)" }
)
...
<body>
  <main>
    ...
    <ul>
      <li>Map size: $CropWCells x $CropHCells cells ($($map.Width) x $($map.Height) total) - compact boss-room map</li>
      <li>Red dot = King Scorpion stand (13, 25) per Crystal GM teleporter</li>
      <li>Orange dot = BDD2 hub entry (30, 35)</li>
      <li>Blue dot = mob respawn cluster</li>
      <li>Green dots = exits back to D2001 cave / forward to BDD 1F</li>
      <li>Reply example: <code>use bdd 2f spot 13, 25</code></li>
    </ul>
    <div class="toolbar">
    ...
    <div class="legend">
      <span><i class="dot" style="background:#e74c3c"></i> King Scorpion</span>
      <span><i class="dot" style="background:#f39c12"></i> BDD2 hub</span>
      <span><i class="dot" style="background:#3498db"></i> Respawn hub</span>
      <span><i class="dot" style="background:#2ecc71"></i> Map exit</span>
    </div>
  </main>
  <script>
    const meta = {
...
# Redirect old card picker to the full-map overview.
$redirectRoots = @(
  (Join-Path $PSScriptRoot "../tile-review/bdd-2f-spot-picker")
)
foreach ($redirectRoot in $redirectRoots) {
  if (-not (Test-Path $redirectRoot)) { continue }
  @"
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <meta http-equiv="refresh" content="0; url=../bdd-2f-overview/index.html" />
  <title>Redirecting...</title>
</head>
<body>
  <p>Moved to <a href="../bdd-2f-overview/index.html">BDD 2F full-map overview</a>.</p>
</body>
</html>
"@ | Set-Content -LiteralPath (Join-Path $redirectRoot "index.html") -Encoding UTF8
}
