...
public sealed class RtzMonsterLib : IDisposable {
  private readonly FileStream stream;
  private readonly BinaryReader reader;
  private readonly int[] offsets;
  public RtzMonsterLib(string path) {
  ...
  public RtzMonsterImage ReadImage(int index) {
    ...
    return new RtzMonsterImage(bitmap, ox, oy);
  }
  public void Dispose() { reader.Dispose(); stream.Dispose(); }
}
public sealed class RtzMonsterImage : IDisposable {
  public Bitmap Bitmap { get; private set; }
  public short OffsetX { get; private set; }
  public short OffsetY { get; private set; }
  public RtzMonsterImage(Bitmap bitmap, short offsetX, short offsetY) { Bitmap = bitmap; OffsetX = offsetX; OffsetY = offsetY; }
...
function Get-MonsterLibActionFrames {
  ...
  $actionNames = @{ 0 = "standing"; 1 = "walking"; 9 = "attack1"; 14 = "attackRange2" }
  $fs = [System.IO.File]::OpenRead($LibraryPath)
  $br = New-Object System.IO.BinaryReader($fs)
  try {
    $null = $br.ReadInt32(); $null = $br.ReadInt32(); $frameSeek = $br.ReadInt32()
    ...
    for ($i = 0; $i -lt $frameCount; $i++) {
      ...
      $null = $br.ReadInt32(); $null = $br.ReadInt32(); $null = $br.ReadInt32(); $null = $br.ReadInt32()
      $reverse = $br.ReadBoolean(); $null = $br.ReadBoolean()
      if (-not $actionNames.ContainsKey($action)) { continue }
      $actions[$actionNames[$action]] = @{ start = $start; count = $count; offset = $count + $skip; interval = $interval; reverse = $reverse }
    }
    return $actions
  }
...
$magicLib = Join-Path $DataRoot "Magic2.Lib"
...
if (-not (Test-Path -LiteralPath $magicLib)) { throw "Magic2 library not found: $magicLib" }
...
$libActions = Get-MonsterLibActionFrames -LibraryPath $monsterLib
$newFrames = New-Object System.Collections.Generic.List[object]
$monsterReader = [RtzMonsterLib]::new((Resolve-Path $monsterLib))
$magicReader = [RtzMonsterLib]::new((Resolve-Path $magicLib))
try {
  if ($atlas.actions.PSObject.Properties.Name -notcontains "attackRange2") {
    $spec = $libActions["attackRange2"]
    if ($null -eq $spec) { throw "attackRange2 missing in monster lib $Index" }
    for ($i = 0; $i -lt $spec.count; $i++) {
      ...
      $newFrames.Add([pscustomobject]@{ action = "attackRange2"; interval = $spec.interval; slot = $nextSlot; srcFrame = $srcFrame; image = $image }) | Out-Null
      $nextSlot += 1
    }
  }

  if (-not $atlas.projectile) {
    for ($i = 0; $i -lt 3; $i++) {
      $srcFrame = 10 + $i
      $image = $magicReader.ReadImage($srcFrame)
      $newFrames.Add([pscustomobject]@{ action = "projectile"; interval = 80; slot = $nextSlot; srcFrame = $srcFrame; image = $image }) | Out-Null
      $nextSlot += 1
    }
  }
}
...
if ($newFrames.Count -eq 0) {
  Write-Host "Monster $Index already has RTZ range attack assets"
  exit 0
}
...
$actions = [ordered]@{}
foreach ($prop in $atlas.actions.PSObject.Properties) { $actions[$prop.Name] = $prop.Value }

foreach ($actionName in @("attackRange2")) {
  ...
  foreach ($frame in $frames) {
    if ($null -eq $frame.image) {
      ...
      $jsonFrames += [ordered]@{
        slot = $frame.slot; srcFrame = $frame.srcFrame
        w = $frame.image.Bitmap.Width; h = $frame.image.Bitmap.Height
        offsetX = $frame.image.OffsetX; offsetY = $frame.image.OffsetY
      }
      $frame.image.Dispose()
    }
  }
  $actions[$actionName] = [ordered]@{ interval = $frames[0].interval; frames = $jsonFrames }
}

$projectile = $atlas.projectile
if (-not $projectile) {
  $projectileFrames = @()
  foreach ($frame in ($newFrames | Where-Object { $_.action -eq "projectile" })) {
    if ($null -eq $frame.image) {
...
Write-Host "Appended RTZ range attack to monster $Index ($($newFrames.Count) new frames)"
