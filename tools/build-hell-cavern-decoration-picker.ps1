param(
  [string]$CatalogRoot = "../tile-review/hell-cavern-prop-catalog",
  [string]$OutputRoot = "../tile-review/hell-cavern-decoration-picker",
  [int]$MaxItems = 0,
  [int]$Columns = 8,
  [int]$CellWidth = 160,
  [int]$CellHeight = 220,
...
try {
  ...
  for ($index = 0; $index -lt $groups.Count; $index++) {
    ...
    <article class="card">
      ...
      <img src="../hell-cavern-prop-catalog/$($group.AssemblyFile)" alt="#$($group.Number)" loading="lazy" />
...
<html>
  <head>
    <meta charset="utf-8" />
    <title>Hell Cavern Decoration Picker</title>
    <style>
  ...
  <body>
    <header>
      <h1>Hell Cavern Decoration Picker</h1>
      <p>All $($groups.Count) lane props from Crystal <code>hell01.map</code> / <code>hell02.map</code> (for optional foreground clutter — not the back wall). Back wall uses corridor edge strips: <a href="../hell-cavern-1-corridor-edge/index.html">HC1 edge</a> · <a href="../hell-cavern-2-corridor-edge/index.html">HC2 edge</a>. Pick catalog numbers here for lane decorations only.</p>
      <p>In-map previews: <a href="../hell-cavern-prop-catalog/index.html">hell-cavern-prop-catalog/index.html</a></p>
    </header>
    <section class="sheet">
      <img src="picker-sheet.png" alt="Hell Cavern decoration picker sheet" />
    </section>
    <section class="grid">
$($cards -join "`n")
    </section>
...
Write-Host "Hell Cavern decoration picker: $sheetPath ($($groups.Count) items)"
