...
try {
  ...
  $atlasJson.projectile.baseIndex = $BaseIndex
  $atlasJson.projectile.slotWidth = $slotWidth
  $atlasJson.projectile.slotHeight = $slotHeight
  $atlasJson.projectile | Add-Member -NotePropertyName frames -NotePropertyValue @($frames.ToArray()) -Force
  $atlasJson | ConvertTo-Json -Depth 20 | Set-Content $atlasPath
  Write-Output "Updated $Atlas projectile to frames $BaseIndex-$($BaseIndex + $Count - 1)"
}
finally {
