param(
  ...
  # Pick catalog numbers from tile-review/prajna-temple-prop-catalog/index.html, then rebuild.
  [int[]]$AssemblyNumbers = @(30, 36, 38, 45, 48, 75, 76, 87, 102),
  [string]$OutputRoot = "../public/mapobjects",
  [string]$SheetName = "prajna-temple-catalog.png",
  [string]$SetId = "prajna-temple-catalog",
  [string]$Label = "Prajna Temple Catalog Decor"
)
...
try {
  ...
  Write-Host "Prajna Temple decoration sheet: $sheetPath ($slotWidth x $slotHeight, $($objects.Count) slots)"
