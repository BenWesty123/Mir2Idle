param(
  ...
  [string]$OutputSheet = "../public/maptiles/zuma-temple.png",
  [string]$SetId = "zuma-temple",
  [string]$Label = "Zuma Temple",
  [int[]]$Frames = @(2401, 2402, 2403, 2404, 2405)
)
...
$byFrame = @{}
foreach ($tile in (Export-TileRange 2398 12 "../tile-review/zuma-temple-tiles-build")) {
...
Write-Host "Zuma Temple tiles: $($tiles.Count) slots ($slotWidth x $slotHeight)"
