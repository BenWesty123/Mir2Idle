param(
  [string]$CatalogRoot = "../tile-review/zuma-temple-prop-catalog",
  [int[]]$AssemblyNumbers = @(63, 61, 47, 41, 5, 6, 1, 10),
  [string]$OutputRoot = "../public/mapobjects",
  [string]$SheetName = "zuma-temple-catalog.png",
  [string]$SetId = "zuma-temple-catalog",
  [string]$Label = "Zuma Temple Catalog Decor"
)
...
try {
  ...
  Write-Host "Zuma decoration sheet: $sheetPath ($slotWidth x $slotHeight, $($objects.Count) slots)"
