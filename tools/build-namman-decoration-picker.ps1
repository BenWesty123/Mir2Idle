param(
  [string]$CatalogRoot = "../tile-review/namman-prop-catalog",
  [string]$OutputRoot = "../tile-review/namman-decoration-picker",
  [int]$MaxItems = 0,
  [int]$Columns = 8,
  [int]$CellWidth = 160,
  [int]$CellHeight = 220,
...
$rows = [Math]::Ceiling($groups.Count / [Math]::Max(1, $Columns))
$sheetWidth = $Columns * $CellWidth
$sheetHeight = [Math]::Max(1, $rows) * ($CellHeight + $LabelHeight)
...
try {
  ...
  for ($index = 0; $index -lt $groups.Count; $index++) {
    ...
    $cards.Add(@"
    <article class="card" data-number="$($group.Number)">
      <header><strong>#$($group.Number)</strong> <span>$($group.Category)</span> <span>used $($group.Count)x</span></header>
      <img src="../namman-prop-catalog/$($group.AssemblyFile)" alt="#$($group.Number)" loading="lazy" />
...
<html>
  <head>
    <meta charset="utf-8" />
    <title>Southern Barbarian Land Decorations Picker</title>
    <style>
      ...
      p { margin: 0 0 6px; color: #b9ad94; max-width: 960px; }
      .sheet { padding: 16px; }
  ...
  <body>
    <header>
      <h1>Southern Barbarian Land — Decorations Picker</h1>
      <p>Props mined from Crystal <code>NAMMAN.map</code> (same WeMade tiles as the looping field floor). Reply with catalog numbers (e.g. <code>#12 #45 #88</code>) for trees / rocks / ruins / stalls to sprinkle on the scrolling field.</p>
      <p>Full catalog with in-map previews: <a href="../namman-prop-catalog/index.html">namman-prop-catalog/index.html</a></p>
      <p>$($groups.Count) props · sorted by how often they appear on the map.</p>
    </header>
    <section class="sheet">
      <img src="picker-sheet.png" alt="Namman decoration picker sheet" />
    </section>
    <section class="grid">
$($cards -join "`n")
    </section>
...
Write-Host "Namman decoration picker: $sheetPath ($($groups.Count) items)"
