param(
  ...
  [string]$PickCommand = "use bdd spot X, Y",
  [string]$HubLink = "../index.html",
  [string[]]$Bullets = @(),
  [array]$Legend = @(),
  [array]$Markers = @(),
  [int]$CropX = 0,
...
$legendHtml = if ($Legend.Count -gt 0) {
  ...
  @"
      <span><i class="dot" style="background:#f39c12"></i> Hub / farming pocket</span>
      <span><i class="dot" style="background:#e74c3c"></i> Boss pocket</span>
      <span><i class="dot" style="background:#3498db"></i> Wave arena candidate</span>
      <span><i class="dot" style="background:#2ecc71"></i> Entry</span>
      <span><i class="dot" style="background:#9b59b6"></i> Exit</span>
"@
}
...
<body>
  <main>
    <p><a href="$HubLink">&larr; All remaining BDD floors</a></p>
    <h1>$MapLabel</h1>
    <p>Full Crystal <code>$mapFileName</code> ($MapTitle) render. <strong>Hover</strong> for map coordinates. <strong>Click</strong> to copy <code>X, Y</code>.</p>
    <ul>
