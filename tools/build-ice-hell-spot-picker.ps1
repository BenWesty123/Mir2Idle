param(
  ...
  [Parameter(Mandatory = $true)][string]$MapTitle,
  [Parameter(Mandatory = $true)][string]$MapLabel,
  [Parameter(Mandatory = $true)][string]$ImagePrefix,
  [Parameter(Mandatory = $true)][string]$ZoneHint,
  [Parameter(Mandatory = $true)][string]$PickWord,   # e.g. "ice hell 1"
...
$stampScript = Join-Path $toolsDir "build-ice-hell-stamp.ps1"
...
# --- Read map back layer so we can auto-place candidate stands on open floor ---
function Read-BackLayer($path) {
...
$map = Read-BackLayer $MapPath
$floorSet = @{}
foreach ($f in $FloorFillFrames) { $floorSet[[int]$f] = $true }

# Collect walkable open-floor cells (even coords, back frame in the floor set).
$floorCells = New-Object System.Collections.Generic.List[object]
$sumX = 0.0; $sumY = 0.0
$minX = [int]::MaxValue; $minY = [int]::MaxValue; $maxX = 0; $maxY = 0
for ($x = 0; $x -lt $map.Width; $x++) {
  if (($x -band 1) -ne 0) { continue }
  for ($y = 0; $y -lt $map.Height; $y++) {
    if (($y -band 1) -ne 0) { continue }
    $b = $map.Back[($x * $map.Height) + $y]
    if ($b -eq 0) { continue }
    $frame = [int](($b -band 0x1FFFFFFF) - 1)
    if (-not $floorSet.ContainsKey($frame)) { continue }
    $floorCells.Add(@($x, $y))
    $sumX += $x; $sumY += $y
...
if ($floorCells.Count -eq 0) { throw "No walkable floor cells found for frames $($FloorFillFrames -join ',') in $MapPath" }

$cellLookup = @{}
foreach ($c in $floorCells) { $cellLookup["$($c[0]),$($c[1])"] = $true }

function Snap-ToFloor([int]$tx, [int]$ty) {
  ...
  for ($r = 0; $r -le 60; $r += 2) {
    for ($dx = -$r; $dx -le $r; $dx += 2) {
      for ($dy = -$r; $dy -le $r; $dy += 2) {
        ...
        $nx = $tx + $dx; $ny = $ty + $dy
        if ($cellLookup.ContainsKey("$nx,$ny")) { return @($nx, $ny) }
      }
    }
  }
  return $null
...
$cx = [int][Math]::Round($sumX / $floorCells.Count)
$cy = [int][Math]::Round($sumY / $floorCells.Count)
$spanX = [Math]::Max(8, [int](($maxX - $minX) * 0.22))
$spanY = [Math]::Max(8, [int](($maxY - $minY) * 0.22))

$targets = @(
  [ordered]@{ id = "center";      label = "Walkable center";       tx = $cx;            ty = $cy;            rec = $true;  note = "Auto-detected centroid of the open floor region. Strong default party stand." },
  [ordered]@{ id = "north";       label = "North of center";       tx = $cx;            ty = $cy - $spanY;   rec = $false; note = "Same open region, one screen north of the centroid." },
  [ordered]@{ id = "south";       label = "South of center";       tx = $cx;            ty = $cy + $spanY;   rec = $false; note = "Same open region, one screen south of the centroid." },
  [ordered]@{ id = "west";        label = "West of center";        tx = $cx - $spanX;   ty = $cy;            rec = $false; note = "Open floor west of the centroid." },
  [ordered]@{ id = "east";        label = "East of center";        tx = $cx + $spanX;   ty = $cy;            rec = $false; note = "Open floor east of the centroid." },
  [ordered]@{ id = "northwest";   label = "North-west pocket";     tx = $cx - $spanX;   ty = $cy - $spanY;   rec = $false; note = "Diagonal open pocket for a different backdrop." },
  [ordered]@{ id = "southeast";   label = "South-east pocket";     tx = $cx + $spanX;   ty = $cy + $spanY;   rec = $false; note = "Diagonal open pocket for a different backdrop." }
)

$candidates = New-Object System.Collections.Generic.List[object]
$seen = @{}
foreach ($t in $targets) {
  $snap = Snap-ToFloor $t.tx $t.ty
...
$legend = @(
  @{ color = "#f39c12"; label = "Recommended stand (walkable center)" },
  @{ color = "#3498db"; label = "Alternate open-floor stands" }
)

$bullets = @(
  "Crystal $([System.IO.Path]::GetFileName($MapPath)) ($MapTitle) - $($map.Width)x$($map.Height)",
  "Candidate stands are auto-placed on open $MapLabel floor (frames $($FloorFillFrames -join ', '))",
  "Crystal has no exported respawn coords for Ice Hell, so pick the party stand from the map yourself",
...
Write-Host "Building $MapLabel full-map overview ($($map.Width)x$($map.Height) - may take several minutes)..."
$overviewJson = & $overviewScript `
  ...
  -PickCommand "use $PickWord spot X, Y" `
  -HubLink "../ice-hell-1-spot-picker/index.html" `
  -Bullets $bullets `
  -Legend $legend `
  -Markers $markers `
  -OverviewScale $OverviewScale | ConvertFrom-Json
...
foreach ($spot in $candidates) {
  ...
  & $stampScript `
    ...
    -FocusMapX $spot.mapX `
    -FocusMapY $spot.mapY `
    -FloorFillFrames $FloorFillFrames | Out-Null
  ...
  $cardHtml = @(
    ...
    '<span class="coords">' + [System.IO.Path]::GetFileName($MapPath) + ' (' + $spot.mapX + ', ' + $spot.mapY + ')</span></header>'
    ...
    '<p class="pick">Reply: <code>use ' + $PickWord + ' spot ' + $spot.id + '</code></p>'
  ) -join "`n"
  $cardHtml += "`n</article>"
  $cards.Add($cardHtml)
}
...
$manifest = [ordered]@{
  title = "$MapLabel ($MapTitle) - spot picker"
...
<head>
  ...
  <title>$MapLabel - spot picker</title>
  <style>
...
<body>
  <header>
    <h1>$MapLabel - group dungeon spot picker</h1>
    <p class="meta">
      Crystal <code>$([System.IO.Path]::GetFileName($MapPath))</code> ($MapTitle). Mock arena for <code>$ZoneHint</code>.
      <strong>Hover</strong> the full map for coordinates. <strong>Click</strong> to copy <code>X, Y</code>.
      Or pick a preset card below: <code>use $PickWord spot &lt;id&gt;</code>.
    </p>
    <ul class="meta">
      <li>Zone hint: <code>$ZoneHint</code></li>
      <li>Recommended default: auto-detected walkable center (see marker)</li>
    </ul>
  </header>
  ...
  <section class="section">
    ...
    <ul class="meta">
$bulletHtml
      <li>Then reply: <code>use $PickWord spot X, Y</code> or <code>use $PickWord spot &lt;id&gt;</code></li>
    </ul>
    <div class="toolbar">
    ...
    <div class="viewer" id="viewer">
      <img id="mapImage" src="$ImagePrefix.png" width="$($overviewJson.scaledWidth)" height="$($overviewJson.scaledHeight)" alt="$MapLabel overview" />
    </div>
    <div class="legend">
$legendHtml
    </div>
