param(
  ...
  [string]$MapPath = "C:/Users/bb-we/Documents/Crystal-master/Next/NextClient/Map/D024.map",
...
function Add-Layer([int]$slotId, [int]$frame, [int]$x, [int]$y, [bool]$floorSized) {
...
for ($pass = 0; $pass -lt 2; $pass++) {
  for ($y = $CropY; $y -lt ($CropY + $CropHCells); $y++) {
    for ($x = $CropX; $x -lt ($CropX + $CropWCells); $x++) {
      ...
      Add-Layer $frontSlot $frontFrame $drawX $drawY $floorSized
    }
  }
}
...
$sheetPath = Join-Path $outRoot "wooma-kr-center-stamp.png"
...
$stamp = [ordered]@{
  id = "wooma-kr-center"
  label = "Wooma KR Center"
  sheet = "wooma-kr-center-stamp.png"
  slotWidth = $slotWidth
...
[ordered]@{ stamps = @($stamp) } | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath (Join-Path $outRoot "index.json") -Encoding UTF8
