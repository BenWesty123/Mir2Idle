param(
  ...
  [string]$OutputRoot = "../tile-review/bdd-d2001-overview",
  [int]$CropX = 0,
  [int]$CropY = 0,
  [int]$CropWCells = 0,
  [int]$CropHCells = 0,
...
$fullPath = Join-Path $outRoot "d2001-overview-full.png"
...
try {
  ...
  $overviewPath = Join-Path $outRoot "d2001-overview.png"
...
<head>
  <meta charset="utf-8" />
  <title>Black Dragon Dungeon — D2001 Overview</title>
  <style>
...
<body>
  <main>
    <h1>Black Dragon Dungeon — D2001 (first cave)</h1>
    <p>Full Crystal map render. <strong>Hover</strong> for map coordinates. <strong>Click</strong> to copy <code>X, Y</code> — paste back as the party stand spot.</p>
    <ul>
      <li>Map size: $CropWCells x $CropHCells cells ($map.Width x $map.Height total)</li>
      <li>Green dot = Castle Gi-Ryoong warp-in ($entryX, $entryY)</li>
      <li>Reply example: <code>use bdd spot 250, 210</code></li>
    </ul>
    <div class="toolbar">
      <div class="coords" id="coords">Hover the map...</div>
      <label><input type="checkbox" id="toggleMarkers" checked /> Show warp-in marker</label>
    </div>
    <div class="viewer" id="viewer">
      <img id="mapImage" src="d2001-overview.png" width="$scaledW" height="$scaledH" alt="BDD D2001 overview" />
    </div>
    <div class="legend">
      <span><i class="dot" style="background:#2ecc71"></i> Gi-Ryoong entry</span>
    </div>
  </main>
  <script>
    const meta = {
      ...
      scale: $OverviewScale,
      dots: [
        { x: $entryX, y: $entryY, className: "entry", title: "Castle Gi-Ryoong warp-in" }
      ],
      rects: []
    };
...
# Replace broken card picker with redirect.
$redirectRoot = Join-Path $PSScriptRoot "../tile-review/bdd-spot-picker"
...
<head>
  <meta charset="utf-8" />
  <meta http-equiv="refresh" content="0; url=../bdd-d2001-overview/index.html" />
  <title>Redirecting...</title>
</head>
<body>
  <p>Moved to <a href="../bdd-d2001-overview/index.html">BDD D2001 overview picker</a>.</p>
</body>
</html>
...
[ordered]@{
  ...
  overviewImage = (Join-Path $outRoot "d2001-overview.png")
  scaledWidth = $scaledW
  scaledHeight = $scaledH
  mapWidth = $map.Width
  mapHeight = $map.Height
