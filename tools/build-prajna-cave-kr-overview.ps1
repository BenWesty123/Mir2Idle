param(
  ...
  [string]$MapPath = "C:/Users/bb-we/Documents/Crystal-master/Next/NextClient/Map/D2067.map",
  [string]$OutputRoot = "../tile-review/prajna-cave-kr-overview",
  [int]$CropX = 0,
  [int]$CropY = 0,
  [int]$CropWCells = 300,
  [int]$CropHCells = 300,
...
$fullPath = Join-Path $outRoot "d2067-overview-full.png"
...
try {
  ...
  $overviewPath = Join-Path $outRoot "d2067-overview.png"
...
} | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $outRoot "meta.json") -Encoding UTF8
...
<head>
  <meta charset="utf-8" />
  <title>Prajna Cave KR — D2067 Overview</title>
  <style>
    ...
    .coords { font-family: Consolas, monospace; background: #1b2029; border: 1px solid #3a4354; padding: 8px 12px; border-radius: 6px; min-width: 280px; }
    ...
    .marker.rect { border: 2px dashed rgba(255,255,255,.85); background: rgba(231,76,60,.12); box-sizing: border-box; }
...
<body>
  <main>
    <h1>Prajna Cave KR — D2067 (6F)</h1>
    <p>Full Crystal map render. <strong>Hover</strong> for map coordinates. <strong>Click</strong> to copy <code>X,Y</code> — tell me where the boss room should be.</p>
    <ul>
      <li>Map size: $CropWCells × $CropHCells cells (300×300)</li>
      <li>Green = GM teleporter entry (153, 165)</li>
      <li>Red = current idle boss focus (142, 184)</li>
      <li>Dashed box = current stamp crop (124,166 → 36×36)</li>
    </ul>
    <div class="toolbar">
    ...
    <div class="viewer" id="viewer">
      <img id="mapImage" src="d2067-overview.png" width="$scaledW" height="$scaledH" alt="Prajna Cave D2067 overview" />
    </div>
    <div class="legend">
      <span><i class="dot" style="background:#2ecc71"></i> GM entry</span>
      <span><i class="dot" style="background:#e74c3c"></i> Current boss focus</span>
      <span>Dashed = current crop</span>
    </div>
  </main>
  <script>
    const meta = {
      ...
      scale: $OverviewScale,
      dots: [
        { x: 153, y: 165, className: "entry", title: "GM teleporter entry" },
        { x: $currentFocusX, y: $currentFocusY, className: "focus", title: "Current boss focus" }
      ],
      rects: [
        { x: $currentCropX, y: $currentCropY, w: $currentCropW, h: $currentCropH, title: "Current stamp crop" }
      ]
    };
    ...
    function renderMarkers() {
      ...
      for (const rect of meta.rects) {
        ...
        const node = document.createElement("div");
        node.className = "marker rect";
        node.style.left = tl.left + "px";
        node.style.top = tl.top + "px";
        node.style.width = (rect.w * meta.cellWidth * meta.scale) + "px";
        node.style.height = (rect.h * meta.cellHeight * meta.scale) + "px";
        node.title = rect.title + " (" + rect.x + ", " + rect.y + " " + rect.w + "x" + rect.h + ")";
        viewer.appendChild(node);
      }
    }
...
[ordered]@{
  ...
  overviewImage = (Join-Path $outRoot "d2067-overview.png")
  scaledWidth = $scaledW
  scaledHeight = $scaledH
} | ConvertTo-Json
