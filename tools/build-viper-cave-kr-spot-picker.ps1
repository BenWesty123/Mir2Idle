param(
  ...
  [string]$MapPath = "C:/Users/bb-we/Documents/Crystal-master/Next/NextClient/Map/SNAKE.map",
  [string]$OutputRoot = "../tile-review/viper-cave-kr-spot-picker",
  [int]$CropWCells = 36,
  [int]$CropHCells = 36,
  [int]$HalfCropW = 18,
  [int]$HalfCropH = 18
...
$stampScript = Join-Path $PSScriptRoot "build-viper-cave-kr-stamp.ps1"
...
# Crystal SNAKE.map (YimoogiNest) - respawns + movements from MirDB export
$markers = @(
  @{ x = 130; y = 130; className = "boss"; title = "Yimoogi boss respawn hub (monsterIndex 143, count 1, spread 130)" },
  @{ x = 219; y = 220; className = "entry"; title = "Warp-in from Viper Cave 2 - SnakePre (13, 263) lands here" },
  @{ x = 219; y = 220; className = "exit"; title = "Exit tile - returns to SnakePre (13, 264)" }
)

$legend = @(
  @{ color = "#e74c3c"; label = "Yimoogi respawn hub (130, 130)" },
  @{ color = "#2ecc71"; label = "Entry / exit from Viper Cave 2 (219, 220)" }
)

$bullets = @(
  "Crystal SNAKE.map (YimoogiNest) - 260x260, light 4",
  "Yimoogi spawns at map center hub (130, 130) with spread 130 (whole nest)",
  "Players warp in from SnakePre at (13, 263) and land at (219, 220)",
  "Exit tile at (219, 220) returns to SnakePre (13, 264)",
...
$candidates = @(
  [ordered]@{
    id = "yimoogi-hub"
    label = "Yimoogi respawn hub"
    mapX = 130
    mapY = 130
    note = "Crystal boss respawn center - Yimoogi x1, delay 120, spread 130. Blue/Yellow Viper lines share this hub."
    recommended = $true
    role = "boss"
  },
  [ordered]@{
    id = "nest-entry"
    label = "Nest entry (from VC2)"
    mapX = 219
    mapY = 220
    note = "Where players appear when entering from SnakePre floor 2 warp (13, 263). Same tile is the exit back to (13, 264)."
    recommended = $false
    role = "entry"
  },
  [ordered]@{
    id = "nest-center-south"
    label = "South of hub"
    mapX = 130
    mapY = 170
    note = "Open nest floor south of the respawn hub - candidate party stand if hub feels too crowded."
    recommended = $false
    role = "wave"
  },
  [ordered]@{
...
Write-Host "Building Yimoogi Nest full-map overview..."
$overviewJson = & $overviewScript `
  -DataRoot $DataRoot `
  -MapPath $MapPath `
  -OutputRoot $OutputRoot `
  -MapTitle "YimoogiNest" `
  -MapLabel "Viper Cave - Yimoogi Nest (SNAKE)" `
  -ImagePrefix "snake-overview" `
  -PickCommand "use viper-cave kr spot X, Y" `
  -HubLink "../viper-cave-prop-catalog/index.html" `
  -Bullets $bullets `
  -Legend $legend `
...
foreach ($spot in $candidates) {
  ...
  $cardHtml = @(
    ...
    '<span class="coords">SNAKE.map (' + $spot.mapX + ', ' + $spot.mapY + ')</span></header>'
    ...
    '<p class="pick">Reply: <code>use viper-cave kr spot ' + $spot.id + '</code></p>'
    '<p class="cmd"><code>powershell -File tools/build-viper-cave-kr-stamp.ps1 -FocusMapX ' + $spot.mapX + ' -FocusMapY ' + $spot.mapY + ' -CropX ' + $cropX + ' -CropY ' + $cropY + ' -CropWCells ' + $CropWCells + ' -CropHCells ' + $CropHCells + '</code></p>'
    '</article>'
  ) -join "`n"
  $cards.Add($cardHtml)
}
...
$manifest = [ordered]@{
  title = "Viper Cave Yimoogi Nest (SNAKE) - spot picker"
  generatedAt = (Get-Date).ToUniversalTime().ToString("o")
  map = "SNAKE.map"
  mapTitle = "YimoogiNest"
  mapLabel = "Viper Cave - Yimoogi Nest"
  zoneHint = "zone-viper-cave-kr"
  cropWCells = $CropWCells
  cropHCells = $CropHCells
...
<head>
  ...
  <title>Viper Cave Yimoogi Nest - spot picker</title>
  <style>
...
<body>
  <header>
    <h1>Viper Cave - Yimoogi Nest spot picker</h1>
    <p class="meta">
      Crystal <code>SNAKE.map</code> (YimoogiNest). Mock KR for <code>zone-viper-cave-kr</code>.
      <strong>Hover</strong> the full map for coordinates. <strong>Click</strong> to copy <code>X, Y</code>.
      Or pick a preset card below: <code>use viper-cave kr spot &lt;id&gt;</code>.
    </p>
    <ul class="meta">
      <li>Zone hint: <code>zone-viper-cave-kr</code></li>
      <li>Yimoogi respawn hub: center <code>(130, 130)</code></li>
      <li>Entry from Viper Cave 2: <code>(219, 220)</code></li>
    </ul>
  </header>
  ...
  <section class="section">
    ...
    <ul class="meta">
$bulletHtml
      <li>Then reply: <code>use viper-cave kr spot X, Y</code> or <code>use viper-cave kr spot &lt;id&gt;</code></li>
    </ul>
    <div class="toolbar">
    ...
    <div class="viewer" id="viewer">
      <img id="mapImage" src="snake-overview.png" width="$($overviewJson.scaledWidth)" height="$($overviewJson.scaledHeight)" alt="Yimoogi Nest overview" />
    </div>
    <div class="legend">
$legendHtml
    </div>
...
Write-Output ([ordered]@{
  ...
  overviewImage = (Join-Path $outRoot "snake-overview.png")
} | ConvertTo-Json)
