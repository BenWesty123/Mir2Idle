...
# Crystal MonsterObject.cs DrawBlend for direction 6 (lane-facing).
$configs = @(
  @{
    Index = 153
    Label = "Cyano Ghast"
    # Attack1: 528 + FrameIndex + Direction * 6
    SrcFrames = @(528, 529, 530, 531, 532, 533) | ForEach-Object { $_ + ($Direction * 6) }
  },
  @{
    Index = 154
    Label = "Mutated Manworm"
    # Attack1: 285 + FrameIndex + Direction * 6
    SrcFrames = @(285, 286, 287, 288, 289, 290) | ForEach-Object { $_ + ($Direction * 6) }
  },
  @{
    Index = 155
    Label = "Crazy Manworm"
    # Crystal only blends Attack2; reuse that slash on our attack1 (272 + FrameIndex + Direction * 8).
    SrcFrames = @(272, 273, 274, 275, 276, 277) | ForEach-Object { $_ + ($Direction * 8) }
  },
  @{
    Index = 159
    Label = "Dark Devourer"
    # Attack1: 352 + FrameIndex + Direction * 6
    SrcFrames = @(352, 353, 354, 355, 356, 357) | ForEach-Object { $_ + ($Direction * 6) }
  }
)
...
function Append-AttackBlend($config) {
  ...
  try {
    for ($i = 0; $i -lt $attackCount; $i++) {
      $src = $srcFrames[$i]
      if ($null -eq $src) {
        $blendFrames += [ordered]@{ slot = $existingSlots + $i; srcFrame = -1; w = 0; h = 0; offsetX = 0; offsetY = 0; empty = $true }
        continue
      }
      $image = $lib.ReadImage([int]$src)
      if ($null -eq $image) {
        Write-Warning "$($config.Label) missing blend frame $src (attack frame $i)"
        $blendFrames += [ordered]@{ slot = $existingSlots + $i; srcFrame = [int]$src; w = 0; h = 0; offsetX = 0; offsetY = 0; empty = $true }
        continue
      }
      $blendFrames += [ordered]@{
  ...
  foreach ($frame in $blendFrames) {
    if ($frame.empty) {
      $jsonBlendFrames += [ordered]@{ slot = $frame.slot; srcFrame = $frame.srcFrame; w = 0; h = 0; offsetX = 0; offsetY = 0; empty = $true }
    } else {
      $jsonBlendFrames += [ordered]@{ slot = $frame.slot; srcFrame = $frame.srcFrame; w = $frame.w; h = $frame.h; offsetX = $frame.offsetX; offsetY = $frame.offsetY }
      $frame.image.Dispose()
    }
  }
  ...
  $actions.attack1Blend = [ordered]@{ interval = $atlas.actions.attack1.interval; frames = @($jsonBlendFrames) }
