param(
  [string]$CatalogRoot = "../tile-review/viper-cave-prop-catalog",
  [int[]]$AssemblyNumbers = @(22, 32, 31, 38, 55, 69, 77, 5, 12, 86),
  [string]$OutputRoot = "../public/mapobjects",
  [string]$SheetName = "viper-cave-catalog.png",
  [string]$SetId = "viper-cave-catalog",
  [string]$Label = "Viper Cave Catalog Decor"
)
...
try {
  ...
  Write-Host "Viper Cave decoration sheet: $sheetPath ($slotWidth x $slotHeight, $($objects.Count) slots)"
