param(
  ...
  [string]$OutputRoot = "../tile-review/blood-land-spot-picker",
  [int]$CropWCells = 36,
  [int]$CropHCells = 36,
  [int]$HalfCropW = 18,
  [int]$HalfCropH = 18,
...
# Crystal 66.map (BloodLand) - 300x300. Planned idle GD floor 5 / Oma King.
# Only warp is SE back to Blood Pass. Oma King (crystal 390) spawns at (150, 150).
$markers = @(
  @{ x = 150; y = 150; className = "boss"; title = "Oma King spawn (map center)" },
  @{ x = 150; y = 90; className = "wave"; title = "North pocket" },
  @{ x = 150; y = 210; className = "wave"; title = "South pocket" },
  @{ x = 90; y = 150; className = "wave"; title = "West pocket" },
  @{ x = 210; y = 150; className = "wave"; title = "East pocket" },
  @{ x = 90; y = 90; className = "wave"; title = "NW pocket" },
  @{ x = 210; y = 90; className = "wave"; title = "NE pocket" },
  @{ x = 90; y = 210; className = "wave"; title = "SW pocket" },
  @{ x = 210; y = 210; className = "wave"; title = "SE pocket" },
  @{ x = 259; y = 210; className = "exit"; title = "Back to Blood Pass (65.map)" }
)

$legend = @(
  @{ color = "#e74c3c"; label = "Oma King spawn (150, 150)" },
  @{ color = "#3498db"; label = "Preset pockets" },
  @{ color = "#9b59b6"; label = "Exit back to Blood Pass" }
)

$bullets = @(
  "Crystal 66.map (BloodLand) - 300x300 outdoor map",
  "Planned Past Bicheon GD floor 5 (Oma King). Click the map, then reply with the coordinates and I will put that crop in the game."
)
...
$candidates = @(
  [ordered]@{
    ...
    mapX = 150
    mapY = 150
    note = "Oma King spawn in Crystal. Middle of Blood Land."
    recommended = $false
    role = "boss"
  },
  [ordered]@{
  ...
  [ordered]@{
    ...
    mapX = 210
    mapY = 150
    note = "East of center, toward Blood Pass."
    recommended = $false
    role = "wave"
  },
  [ordered]@{
  ...
  [ordered]@{
    ...
    mapX = 210
    mapY = 210
    note = "SE pocket, near the Blood Pass exit."
    recommended = $false
    role = "wave"
  },
  [ordered]@{
    ...
    mapX = 259
    mapY = 210
    note = "Warp back to Blood Pass (65.map). Exit tile, not a stand."
    recommended = $false
    role = "exit"
  }
)
...
Write-Host "Building Blood Land full-map overview (300x300)..."
$overviewJson = & $overviewScript `
  ...
  -MapTitle "BloodLand" `
  -MapLabel "Blood Land" `
  -ImagePrefix "bloodland-overview" `
  -PickCommand "use oma king spot X, Y" `
  -HubLink "./index.html" `
  -Bullets $bullets `
  -Legend $legend `
...
foreach ($spot in $candidates) {
  ...
  $roleBadge = switch ($spot.role) {
    "boss" { '<span class="badge boss">Oma King</span>' }
    "wave" { '<span class="badge wave">Pocket</span>' }
    "exit" { '<span class="badge exit">Exit</span>' }
    default { "" }
  }
  $stampCmd = 'powershell -File tools/build-bdd-1f-stamp.ps1 -MapPath "' + ($MapPath -replace '\\','/') + '" -StampId "blood-land-gd-5-center" -StampLabel "Blood Land GD Floor 5" -FocusMapX ' + $spot.mapX + ' -FocusMapY ' + $spot.mapY + ' -CropX ' + $cropX + ' -CropY ' + $cropY + ' -CropWCells ' + $CropWCells + ' -CropHCells ' + $CropHCells
  $cardHtml = @(
    '<article class="card" id="spot-' + $spot.id + '">'
    '<header><strong>' + $spot.label + '</strong> ' + $rec + ' ' + $roleBadge
    '<span class="coords">66.map (' + $spot.mapX + ', ' + $spot.mapY + ')</span></header>'
    ...
    '<p class="pick">Reply: <code>use oma king spot ' + $spot.mapX + ', ' + $spot.mapY + '</code></p>'
...
$manifest = [ordered]@{
  title = "Blood Land (BloodLand) - Oma King spot picker"
  generatedAt = (Get-Date).ToUniversalTime().ToString("o")
  map = "66.map"
  mapTitle = "BloodLand"
  mapLabel = "Blood Land"
  zoneHint = "zone-past-bicheon-gd-5 (planned GD floor 5)"
  stampHint = "blood-land-gd-5-center"
  cropWCells = $CropWCells
  cropHCells = $CropHCells
...
<head>
  ...
  <title>Blood Land - Oma King spot picker</title>
  <style>
...
<body>
  <header>
    <h1>Blood Land - Oma King spot picker</h1>
    <p class="meta">
      Crystal <code>66.map</code> (<strong>BloodLand</strong>) - 300x300.
      Planned Past Bicheon GD floor 5.
    ...
    <ul class="meta">
      <li>Oma King spawn: <code>(150, 150)</code></li>
      <li>Back to Blood Pass: <code>(259, 210)</code></li>
    </ul>
  </header>
  ...
  <section class="section">
    ...
    <ul class="meta">
$bulletHtml
      <li>Then reply: <code>use oma king spot X, Y</code></li>
    </ul>
    <div class="toolbar">
    ...
    <div class="viewer" id="viewer">
      <img id="mapImage" src="bloodland-overview.png" width="$($overviewJson.scaledWidth)" height="$($overviewJson.scaledHeight)" alt="Blood Land overview" />
    </div>
    <div class="legend">
$legendHtml
    </div>
...
$markersJson
    ...
    img.addEventListener("click", async (event) => {
      ...
      coords.textContent = "Copied: " + text + "  ->  use oma king spot " + text;
...
Write-Output ([ordered]@{
  ...
  overviewImage = (Join-Path $outRoot "bloodland-overview.png")
} | ConvertTo-Json)
