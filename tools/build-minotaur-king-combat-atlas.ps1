...
$actions = [ordered]@{
  ...
  attack1Blend = @{ start = 272; count = 6; offset = 6; interval = 100; directional = $true }
}
...
try {
  ...
  for ($i = 0; $i -lt $config.projectileCount; $i++) {
    Add-Frame -Action "projectile" -SrcFrame ($config.projectileStart + $i) -Lib $lib -Projectile
  }
...
Write-Output "Built Minotaur King image=$($config.imageIndex) -> $($config.outputIndex) ($($frames.Count) slots, ${slotWidth}x${slotHeight})"
