param(
  ...
  [string]$MapPath = "C:/Users/bb-we/Documents/Crystal-master/Next/NextClient/Map/D2079.map",
...
function Get-VisibleBackFrame([int]$backFrame) {
  # Prajna Temple maps store 3100-3104 directly; older exports use 1950+1150.
  if ($backFrame -ge 3100 -and $backFrame -le 3104) { return $backFrame }
  if ($backFrame -ge 1950 -and $backFrame -le 1999) { return $backFrame + 1150 }
...
$PRAJNA_FLOOR_FRAMES = @(3100, 3101, 3102, 3103, 3104)
...
$sheetPath = Join-Path $outRoot "prajna-temple-kr-center-stamp.png"
...
$stamp = [ordered]@{
  id = "prajna-temple-kr-center"
  label = "Prajna Temple KR Center"
  sheet = "prajna-temple-kr-center-stamp.png"
  slotWidth = $slotWidth
...
if (Test-Path $indexPath) {
  ...
  $existingStamps = @($parsed.stamps | Where-Object { $_.id -ne "prajna-temple-kr-center" })
}
