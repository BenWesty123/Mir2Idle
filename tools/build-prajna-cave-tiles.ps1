param(
  ...
  [string]$OutputSheet = "../public/maptiles/prajna-cave.png",
  [string]$SetId = "prajna-cave",
  [string]$Label = "Prajna Cave",
  # Crystal D2061/D2062 walkable back tiles. 1950-1954 export black in Tiles.Lib;
  # the visible Prajna stone floor is the 3051-3055 group (same as stone-temple note on 195x).
  [int[]]$Frames = @(3051, 3052, 3053, 3054, 3055)
)
...
$byFrame = @{}
foreach ($tile in (Export-TileRange 3048 12 "prajna-cave-tiles-build")) {
...
Write-Host "Prajna Cave tiles: $($tiles.Count) slots ($slotWidth x $slotHeight)"
