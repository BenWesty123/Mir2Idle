param(
  ...
  [int[]]$AssemblyNumbers = @(),
  # Pick catalog numbers from tile-review/hell-cavern-decoration-picker/index.html, then rebuild.
  [string]$OutputRoot = "../public/mapobjects",
  [string]$SheetName = "hell-cavern-catalog.png",
  [string]$SetId = "hell-cavern-catalog",
  [string]$Label = "Hell Cavern Catalog Decor"
)
...
if ($AssemblyNumbers.Count -eq 0) {
  throw "AssemblyNumbers is empty. Open tile-review/hell-cavern-decoration-picker/index.html, pick catalog numbers, then pass -AssemblyNumbers @(22, 32, ...)"
}
...
try {
  ...
  Write-Host "Hell Cavern decoration sheet: $sheetPath ($slotWidth x $slotHeight, $($objects.Count) slots)"
