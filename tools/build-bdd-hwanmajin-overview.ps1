param(
  ...
  [string]$MapPath = "C:/Users/bb-we/Documents/Crystal-master/Next/NextClient/Map/D2007.map",
  [string]$OutputRoot = "../tile-review/bdd-hwanmajin-overview",
  [string]$MapTitle = "HwanMaJin",
  [string]$MapLabel = "HwanMaJin (BDD 6F)",
  [string]$ImagePrefix = "d2007-overview",
  [int]$CropX = 0,
  [int]$CropY = 0,
  [int]$CropWCells = 0,
  [int]$CropHCells = 0,
...
# Crystal HwanMaJin (D2007) - next room after Wooma Palace North (D2006 exit).
$markers = @(
  @{ x = 50; y = 50; className = "hub"; title = "West respawn cluster - Ghoul20 / ZumaArcher20 / WhiteBoar20" },
  @{ x = 131; y = 144; className = "spawn"; title = "East section wave arena (BDD 6F)" },
  @{ x = 150; y = 150; className = "boss"; title = "East respawn cluster + boss spawns - ZumaStatue21 / ZumaGuardian21 / WhiteBoar21" },
  @{ x = 15; y = 22; className = "entry"; title = "Entry from Wooma Palace (D2006)" },
  @{ x = 19; y = 22; className = "entry"; title = "Entry from Purgatory Hall (D2005)" },
  @{ x = 20; y = 86; className = "exit"; title = "Exit forward to Noble Hog Palace (D2008)" },
  @{ x = 16; y = 21; className = "exit"; title = "Exit back to Wooma Palace (D2006)" }
)
...
<body>
  <main>
    ...
    <ul>
      ...
      <li>Orange dot = west hub (50, 50) - Ghoul20 / ZumaArcher20 / WhiteBoar20 cluster</li>
      <li>Red dot = east hub (150, 150) - main farming + ZumaStatue21 / ZumaGuardian21 / WhiteBoar21</li>
      <li>Green dots = entries from Wooma Palace (15, 22) and Purgatory (19, 22)</li>
      <li>Purple dots = exit back to Wooma (16, 21) and forward to Noble Hog Palace (20, 86)</li>
      <li>Click anywhere to copy coordinates, then reply: <code>use bdd hwanmajin spot X, Y</code></li>
    </ul>
    <div class="toolbar">
    ...
    <div class="legend">
      <span><i class="dot" style="background:#f39c12"></i> West hub (50, 50)</span>
      <span><i class="dot" style="background:#e74c3c"></i> East hub + bosses (150, 150)</span>
      <span><i class="dot" style="background:#2ecc71"></i> Entry</span>
      <span><i class="dot" style="background:#9b59b6"></i> Exits</span>
    </div>
  </main>
  <script>
    const meta = {
