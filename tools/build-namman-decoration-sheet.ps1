param(
  ...
  # Pick catalog numbers from tile-review/namman-decoration-picker/index.html, then rebuild.
  [int[]]$AssemblyNumbers = @(56, 46, 80, 86, 155, 296),
  [string]$OutputRoot = "../public/mapobjects",
  [string]$SheetName = "namman-catalog.png",
  [string]$SetId = "namman-catalog",
  [string]$Label = "Southern Barbarian Catalog Decor"
)
...
if ($AssemblyNumbers.Count -eq 0) {
  throw "AssemblyNumbers is empty. Open tile-review/namman-decoration-picker/index.html, pick catalog numbers, then pass -AssemblyNumbers @(56, 46, ...)"
}
...
try {
  ...
  Write-Host "Namman decoration sheet: $sheetPath ($slotWidth x $slotHeight, $($objects.Count) slots)"
