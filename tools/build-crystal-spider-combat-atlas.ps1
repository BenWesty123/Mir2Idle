...
# Crystal CrystalSpider (Mon61):
#   Attack1 @80 x6 — melee
#   Attack2 @224 x6 — ranged LineAttack body (ObjectAttack Type=1)
#   DrawBlend/effect after Attack2 dirs: start 272, 10 frames/dir (4 wind-up + 6 wide beam)
#   Dir6 beam strip: 336..341 (644px line, same role as King Scorpion attached projectile)
...
$effectBase = 272 + ($Direction * 10)
$beamStart = $effectBase + 4
...
$actions = [ordered]@{
  ...
  # Crystal ObjectAttack Type=1 → Attack2; also expose as attackRange1 for ranged helpers.
  attack2 = @{ start = 224; count = 6; offset = 6; interval = 100 }
  attackRange1 = @{ start = 224; count = 6; offset = 6; interval = 100 }
  ...
  revive = @{ start = 144; count = 10; offset = 10; interval = 100; reverse = $true }
  projectile = @{ start = $beamStart; count = 6; offset = 0; interval = 30 }
}
...
Write-Output "Built Crystal Spider combat atlas $Index ($($frames.Count) slots, ${slotWidth}x${slotHeight}; beam src $beamStart..$($beamStart+5))"
