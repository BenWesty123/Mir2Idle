param(
  ...
  [string]$MapPath = "C:/Users/bb-we/Documents/Crystal-master/Next/NextClient/Map/D10053.map",
  [string]$OutputRoot = "../tile-review/red-evil-ape-spot-picker",
  [int]$CropWCells = 36,
  [int]$CropHCells = 36,
  [int]$HalfCropW = 18,
  [int]$HalfCropH = 18,
...
# Crystal D10053 (RedValley_5F pocket, 100x100) - Red Evil Ape KR
# Idle zone-red-valley-gd-4 / Red Moon Valley - Ape Den (groupDungeonFloor 5).
# MirDB hub (75,75) spread 75: RedEvilApe + CrystalSpider + SpiderBat/Great/Root/BigApe.
$markers = @(
  @{ x = 75; y = 75; className = "boss"; title = "Red Evil Ape hub (spread 75) + CrystalSpider x1 + spider/ape trash" },
  @{ x = 41; y = 91; className = "exit"; title = "Exit SW to RedValley_5F D10051 (11, 27)" }
)

$legend = @(
  @{ color = "#e74c3c"; label = "Red Evil Ape hub (75, 75)" },
  @{ color = "#9b59b6"; label = "Exit to D10051 (5F main)" }
)

$bullets = @(
  "Crystal D10053.map (RedValley_5F pocket) - 100x100 - Red Evil Ape KR",
  "Idle zone: zone-red-valley-gd-4 (Red Moon Valley - Ape Den)",
  "MirDB hub (75, 75) spread 75: RedEvilApe / CrystalSpider / SpiderBat / GreatSpider / RootSpider / BigApe",
  "Idle fight is Red Evil Ape only (id 464)",
...
$candidates = @(
  [ordered]@{
    id = "ape-hub"
    label = "Red Evil Ape hub"
    mapX = 75
    mapY = 75
    note = "MirDB hub (75, 75)."
    recommended = $false
    role = "boss"
  },
  [ordered]@{
    id = "stand-78-24"
    label = "Live stand (78, 24)"
    mapX = 78
    mapY = 24
    note = "Selected party stand for zone-red-valley-gd-4."
    recommended = $true
    role = "boss"
  },
  [ordered]@{
    ...
    mapX = 75
    mapY = 55
    note = "North of center on the small pocket map."
    recommended = $false
    role = "wave"
  },
  [ordered]@{
    ...
    mapX = 75
    mapY = 90
    note = "South of center toward the D10051 exit."
    recommended = $false
    role = "wave"
  },
  [ordered]@{
  ...
  [ordered]@{
    id = "hub-sw"
    label = "Southwest toward exit"
    mapX = 50
    mapY = 88
    note = "Toward the D10051 warp at (41, 91)."
    recommended = $false
    role = "wave"
  },
  [ordered]@{
    id = "exit-5f-sw"
    label = "Exit to 5F main (D10051)"
    mapX = 41
    mapY = 91
    note = "Warp to RedValley_5F D10051 - layout reference, not a fight stand."
    recommended = $false
    role = "exit"
  }
)
...
Write-Host "Building Red Evil Ape KR overview (D10053 RedValley_5F pocket, ${MapSize}x${MapSize})..."
$overviewJson = & $overviewScript `
  -DataRoot $DataRoot `
  -MapPath $MapPath `
  -OutputRoot $OutputRoot `
  -MapTitle "RedValley_5F" `
  -MapLabel "Red Moon Valley - Red Evil Ape KR" `
  -ImagePrefix "d10053-overview" `
  -PickCommand "use red evil ape spot X, Y" `
  -HubLink "./index.html" `
  -Bullets $bullets `
  -Legend $legend `
...
foreach ($spot in $candidates) {
  ...
  $stampCmd = 'powershell -File tools/build-bdd-1f-stamp.ps1 -MapPath "' + ($MapPath -replace '\\','/') + '" -StampId "red-valley-gd-4-center" -FocusMapX ' + $spot.mapX + ' -FocusMapY ' + $spot.mapY + ' -CropX ' + $cropX + ' -CropY ' + $cropY + ' -CropWCells ' + $CropWCells + ' -CropHCells ' + $CropHCells
  $cardHtml = @(
    '<article class="card" id="spot-' + $spot.id + '">'
    '<header><strong>' + $spot.label + '</strong> ' + $rec + ' ' + $roleBadge
    '<span class="coords">D10053.map (' + $spot.mapX + ', ' + $spot.mapY + ')</span></header>'
    ...
    '<p class="pick">Reply: <code>use red evil ape spot ' + $spot.id + '</code></p>'
...
$manifest = [ordered]@{
  title = "Red Evil Ape KR (D10053) - spot picker"
  generatedAt = (Get-Date).ToUniversalTime().ToString("o")
  map = "D10053.map"
  mapTitle = "RedValley_5F"
  mapLabel = "Red Moon Valley - Red Evil Ape KR"
  zoneHint = "zone-red-valley-gd-4"
  stampHint = "red-valley-gd-4-center"
  cropWCells = $CropWCells
  cropHCells = $CropHCells
...
<head>
  ...
  <title>Red Evil Ape KR - spot picker</title>
  <style>
...
<body>
  <header>
    <h1>Red Evil Ape KR - spot picker</h1>
    <p class="meta">
      Crystal <code>D10053.map</code> (<strong>RedValley_5F</strong> pocket, 100x100) -
      Red Evil Ape mini-boss room. Idle zone <code>zone-red-valley-gd-4</code>.
      <strong>Hover</strong> for coordinates. <strong>Click</strong> to copy <code>X, Y</code>.
      Or pick a card: <code>use red evil ape spot &lt;id&gt;</code>.
    </p>
    <ul class="meta">
      <li>Recommended / live: hub <code>(75, 75)</code></li>
      <li>Previous: <a href="../red-valley-3f-spot-picker/index.html">Red Moon Valley 3F (D10031)</a></li>
  ...
  <section class="section">
    ...
    <ul class="meta">
$bulletHtml
      <li>Then reply: <code>use red evil ape spot X, Y</code> or <code>use red evil ape spot &lt;id&gt;</code></li>
    </ul>
    <div class="toolbar">
    ...
    <div class="viewer" id="viewer">
      <img id="mapImage" src="d10053-overview.png" width="$($overviewJson.scaledWidth)" height="$($overviewJson.scaledHeight)" alt="Red Evil Ape KR overview" />
    </div>
    <div class="legend">
$legendHtml
    </div>
...
$markersJson
    ...
    img.addEventListener("click", async (event) => {
      ...
      coords.textContent = "Copied: " + text + "  ->  use red evil ape spot " + text;
...
Write-Output ([ordered]@{
  ...
  overviewImage = (Join-Path $outRoot "d10053-overview.png")
} | ConvertTo-Json)
